...
class UdacitySimEnv(gym.Env):
    ...
    def compute_reward(self, speed, is_off, action):
        ...
        if is_off:
            ...
            reward += 1  # stay on track reward

        # Speed reward shaping
        if speed < 10.0:
            reward -= 2.0  # strong penalty for very slow
        elif speed < 15.0:
            reward += (speed - 10.0) * 0.2  # gradual reward up to 15
        elif speed <= 25.0:
            reward += 1.0  # solid reward for good speed
        else:
            reward += 1.5  # bonus reward for high-speed cruising

        # Encourage speed stability
        if len(self.speed_history) >= 5:
            speed_std = np.std(self.speed_history)
            reward += max(0, 0.5 - speed_std)  # higher reward for stable speed
